...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        # Guardar el estado actual antes de la actualización para verificar si cambió
        old_status = payment.status
        old_cuatrimestre_enrollment = payment.cuatrimestre_enrollment
        
        # Pasar el usuario actual al modelo para que pueda usarlo en save()
        if user:
            payment._current_user = user
        
        # Guardar el pago (el modelo manejará la aprobación automática si corresponde)
        serializer.save()
        
        # Verificar si el pago se aprobó automáticamente
        payment.refresh_from_db()
        if payment.status != old_status and payment.status == 'APROBADO':
            # El pago fue aprobado automáticamente, todo está bien
            pass
        elif not payment.cuatrimestre_enrollment and payment.status != 'APROBADO' and payment.status != 'RECHAZADO':
            # Si no se aprobó automáticamente pero debería, forzar la aprobación
            payment.status = 'APROBADO'
            if not payment.approved_by and user:
    ...
    def find_oldest_unpaid(self, request):
        """Encontrar el pago más antiguo sin pagar (no aprobado) para un estudiante y tipo de pago"""
        student_id = request.query_params.get('student_id')
        ...
        try:
            # Primero verificar si hay pagos para este estudiante y tipo de pago
            all_payments = Payment.objects.filter(
                student_id=student_id,
                payment_type_id=payment_type_id
            )
            
            # Obtener información de depuración sobre todos los pagos
            total_count = all_payments.count()
            status_counts = {}
            for status_choice in ['PENDIENTE', 'EN_REVISION', 'APROBADO', 'RECHAZADO']:
                status_counts[status_choice] = all_payments.filter(status=status_choice).count()
            
            # Buscar el pago más antiguo que no esté aprobado
            # Buscar por cualquier status que no sea APROBADO
            from django.db.models import F, Case, When, IntegerField
            
            unpaid_payment = all_payments.exclude(
                status='APROBADO'
            ).annotate(
                # Ordenar por año (si es None, usar 0), luego mes (si es None, usar 0)
                sort_year=Case(
                    When(year__isnull=True, then=0),
                    default=F('year'),
            ...
            ).order_by('sort_year', 'sort_month', 'payment_date').first()
            ...
            # Calcular monto final (amount + penalty_amount si existe)
            final_amount = unpaid_payment.amount
            if unpaid_payment.penalty_amount and unpaid_payment.penalty_amount > Decimal('0.00'):
                final_amount = unpaid_payment.amount + unpaid_payment.penalty_amount
    ...
    def download_receipt(self, request, pk=None):
        ...
        try:
            ...
            logger.error(f'Error al generar recibo PDF para pago {payment.id}: {str(e)}\n{error_traceback}')
            return Response(
    ...
    def students_with_overdue(self, request):
        """Obtener estudiantes con pagos vencidos"""
        try:
            from django.utils import timezone
            
            today = timezone.now().date()
            
            # Obtener pagos vencidos (con fecha límite pasada y no aprobados, o con mora)
            overdue_payments = Payment.objects.filter(
                Q(
                    # Pagos con fecha límite pasada y no aprobados
                    Q(due_date__lt=today, status__in=['PENDIENTE', 'EN_REVISION']) |
                    # O pagos con mora aplicada
                    Q(penalty_amount__gt=Decimal('0.00'))
                ),
                ...
                total_overdue_amount=Sum('amount'),
                overdue_payments_count=Count('id')
            ).order_by('-total_overdue_amount')
            
            students_list = []
            for item in overdue_payments:
                student_name = f"{item['student__first_name'] or ''} {item['student__first_last_name'] or ''} {item['student__second_last_name'] or ''}".strip()
                students_list.append({
                    'student_id': str(item['student__id']),
                    'student_name': student_name,
                    'student_phone': item['student__phone'] or 'N/A',
                    'total_overdue_amount': float(item['total_overdue_amount'] or Decimal('0.00')),
                    'overdue_payments_count': item['overdue_payments_count'] or 0
                })
            ...
            print(f"Error in students_with_overdue endpoint: {error_detail}")
            print(traceback_str)
            return Response(
                {'error': 'Error al obtener estudiantes con mora', 'detail': error_detail},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def statistics(self, request):
        ...
        try:
            ...
            # Usar values('payment_date') directamente en lugar de TruncDate para mejor compatibilidad con SQLite
            try:
                daily_payments_qs = approved_payments.filter(
                    payment_date__gte=last_15_days_start.date()
                ).values('payment_date').annotate(
                    total=Sum('amount'),
                    count=Count('id')
                ).order_by('payment_date')
                # Convertir a lista inmediatamente para evitar problemas con SQLite
                daily_payments = list(daily_payments_qs)
            except Exception as e:
                # Si falla, usar una lista vacía y registrar el error
                import logging
                logger = logging.getLogger(__name__)
                logger.error(f'Error al obtener pagos diarios: {str(e)}')
                daily_payments = []
...
def process_public_payment(request):
    ...
    # Los pagos con tarjeta se aprueban automáticamente
    from django.utils import timezone
    payment = Payment.objects.create(
        ...
        status='APROBADO',  # Los pagos con tarjeta se aprueban automáticamente
        approved_at=timezone.now(),  # Fecha de aprobación automática
